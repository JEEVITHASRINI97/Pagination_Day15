...
//div element contains pagination data
let detail = document.createElement('div')
detail.classList.add('detail')
document.body.append(detail)

//Create a p tag for id
let idElement = document.createElement('p')
idElement.classList.add('id')
let id = document.getElementsByClassName('detail')[0].insertAdjacentElement('beforeend', idElement)

//Create a p tag for name
let nameElement = document.createElement('p')
nameElement.classList.add('name')
let name = document.getElementsByClassName('detail')[0].insertAdjacentElement('beforeend', nameElement)

//Create a p tag for email
let emailElement = document.createElement('p')
emailElement.classList.add('email')
let email = document.getElementsByClassName('detail')[0].insertAdjacentElement('beforeend', emailElement)

//div element for pagination UI
let addDiv = document.createElement('div')
addDiv.classList.add('main-container')
document.body.append(addDiv)

// next button
let next = document.createElement('div')
next.classList.add('next')
next.setAttribute('onclick', 'nextpage(this)')

//previous button
let previous = document.createElement('div')
previous.classList.add('previous')
previous.setAttribute('onclick', 'previouspage(this)')

//First button
let first = document.createElement('div')
first.classList.add('first')
first.setAttribute('onclick', 'firstpage(this)')

//insert next and previous button inside the main-container
let main = document.querySelectorAll('.main-container')[0]
main.insertAdjacentElement('afterbegin', next)
main.insertAdjacentElement('afterbegin', previous)
main.insertAdjacentElement('afterbegin', first)

document.querySelectorAll('.next')[0].innerText = 'Next'
document.querySelectorAll('.previous')[0].innerText = 'Previous'
document.querySelectorAll('.first')[0].innerText = 'First'

//Add pagination data
for (let i = 1; i < 11; i++) {
	let add = document.createElement('div')
	add.classList.add('link')
	let check = document.getElementsByClassName('main-container')[0].insertAdjacentElement('beforeend', add)
	check.innerText = i
	check.setAttribute('onclick', 'pagination(this)')
}

//logic for pagination to show data inside the div element
function pagination(e) {
	if (parseInt(e.innerText) <= 0 || parseInt(e.innerText) >= data.length + 1) {
		return alert('No data')
	}
	document.getElementsByClassName('id')[0].innerText = ` Id = ${data[parseInt(e.innerText) - 1].id}`
	document.getElementsByClassName('name')[0].innerText = ` Name = ${data[parseInt(e.innerText) - 1].name}`
	document.getElementsByClassName('email')[0].innerText = ` Email = ${data[parseInt(e.innerText) - 1].email}`
	console.log(e.key)
	let checkNumber = parseInt(e.innerText) / 10
	let cal = parseInt(e.innerText) + 10
	if (checkNumber === 0 || (Number.isInteger(checkNumber) && e.key !== '100')) {
		let j = 0
		let i = parseInt(e.innerText) + 1
		while (parseInt(e.innerText) <= cal) {
			document.getElementsByClassName('link')[j].innerText = i
			j++
			i++
		}
	}
...
function nextpage(e) {
	console.log(e)
	let lastpagedata = document.getElementsByClassName('link')[0].innerText
	let checklimit = document.getElementsByClassName('link')[8].innerText
	if (parseInt(checklimit) !== 9 || parseInt(checklimit) !== data.length - 1) {
		let i = 0
		let j = 1
		while (parseInt(lastpagedata) < parseInt(lastpagedata) + 10) {
			document.getElementsByClassName('link')[i].innerText = parseInt(lastpagedata) + j
			i++
			j++
		}
	}
...
function previouspage(e) {
	console.log(e)
	let lastpagedata = document.getElementsByClassName('link')[9].innerText
	let checklimit = document.getElementsByClassName('link')[8].innerText
	if (parseInt(checklimit) !== 9 || parseInt(checklimit) !== data.length - 1) {
		let i = 9
		let j = 1
...
function firstpage(e) {
	let j = 0
	for (let i = 1; i < 11; i++) {
		let check = document.getElementsByClassName('link')[j]
		check.innerText = i
		j++
	}
}
